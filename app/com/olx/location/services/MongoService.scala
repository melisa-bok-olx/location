...
trait MongoService {
  ...
  def saveLocationUser(locationUser: LocationUserModel): Future[Try[Option[LocationUserModel]]]
  def findLocationUser(email: String): Future[Try[Option[LocationUserModel]]]

  def saveLocationTrack(locationTrack: LocationTrackModel): Future[Try[Option[LocationTrackModel]]]
  def findLocationTracks(email: String, pageSize: Int, offset: Int): Future[Try[List[LocationTrackModel]]]
}
...
class MongoServiceImpl(mongoDatabase: MongoDB) extends MongoService {
  ...
  def saveLocationUser(locationUser: LocationUserModel): Future[Try[Option[LocationUserModel]]] = Future {
    Try {
  ...
  def findLocationUser(email: String): Future[Try[Option[LocationUserModel]]] = Future {
    Try {
  ...
  def saveLocationTrack(locationTrack: LocationTrackModel): Future[Try[Option[LocationTrackModel]]] = Future {
    Try {
  ...
  def findLocationTracks(email: String, pageSize: Int, offset: Int): Future[Try[List[LocationTrackModel]]] = Future {
    Try {
